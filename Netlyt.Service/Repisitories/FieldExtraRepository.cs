...
{
    public class FieldExtraRepository : IFieldExtraRepository
    {
        ...
        public FieldExtraRepository(IAmbientDbContextLocator ambientDbContextLocator)
        {
            if (ambientDbContextLocator == null) throw new ArgumentNullException(nameof(ambientDbContextLocator));
            _ambientDbContextLocator = ambientDbContextLocator;
        }

        public FieldExtras Get(long id)
        {
            return DbContext.FieldExtras.Find(id);
        }

        public FieldExtra GetByKey(long fieldExtrasId, string key)
        {
            return DbContext.FieldExtra.FirstOrDefault(x => x.FieldExtrasId == fieldExtrasId
                                                            && x.Key == key);
        }
    }
}
