...
namespace Netlyt.ServiceTests.Lex.Generators
{
    ...
    public class DonutScriptCodeGeneratorTests
    {
        ...
        public DonutScriptCodeGeneratorTests(DonutConfigurationFixture fixture)
        {
...
WEEKDAY(first_Romanian_time)";
        ...
        [Fact]
        public void GenerateDonutTest()
        {
            string assemblyName = "asmxt";
            var generatedDonut = _codeGen.GenerateDonut(assemblyName, _dscript);
            var correctCode = DonutConfigurationFixture.GetTemplate("GeneratedDonutTest1.txt");
            
            var hashCode = HashAlgos.Adler32(generatedDonut);
            Assert.Equal((uint)3628015793, hashCode);
        }
    }
}
